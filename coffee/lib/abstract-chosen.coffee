class AbstractChosen
  ...
  container_width: ->
    return if @options.width? then (if String(@options.width).match(/[-+]\d+/) then "#{(@form_field.offsetWidth+parseInt(@options.width))}px" else @options.width) else "#{@form_field.offsetWidth}px"
